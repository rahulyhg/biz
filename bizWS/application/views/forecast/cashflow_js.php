<script>
            ...
            if (errorFlag) {
                ...
                $.ajax({
                    ...
                    beforeSend: function () {
                        ...
                        if ($('.cashflownxt').length > 0) {
                            $('.cashflownxt').remove();
                        }
                    },
                    success: function (data) {
                        $('.cashflowNext').html(data);
                        var html = '&nbsp<a href="' + baseUrl + 'forecast/balancesheet/' + companyid + '"><button class="btn btn-warning cashflownxt">Next</button></a>';
                        $('.cashflowNext').after(html);
                    }
                });
            }
        });
    ...
    function generateAjaxTotal(cashflow) {
        var ajaxObj = {
            ...
            fy4: ($('b.grant' + cashflow + 'TotalFy4').length > 0) ? $('b.grant' + cashflow + 'TotalFy4').html() : 0,
            fy5: ($('b.grant' + cashflow + 'TotalFy5').length > 0) ? $('b.grant' + cashflow + 'TotalFy5').html() : 0,
        };
        return ajaxObj;
    }
    //generate ajax data
    function generateAjaxData(cashflow) {
        ...
        $('tr.' + cashflow + 'Cashflow').each(function (i, j) {
            ...
            ajaxObj[i] = {
                ...
                fy4: (ele.find('input[name^="' + cashflow + 'CashflowFy4"]').length > 0) ? ele.find('input[name^="' + cashflow + 'CashflowFy4"]').val() : 0,
                fy5: (ele.find('input[name^="' + cashflow + 'CashflowFy5"]').length > 0) ? ele.find('input[name^="' + cashflow + 'CashflowFy5"]').val() : 0,
